// Shared MCP server data — used by both API routes and internal calls
...
export const mcpServers: Record<string, ServerConfig> = {
  library: {
    ...
    tools: [
      { name: "search_books", description: "Search the library catalog by title, author, or category", inputSchema: { type: "object", properties: { query: { type: "string", description: "Search query" }, category: { type: "string" }, available_only: { type: "boolean" } }, required: ["query"] } },
      { name: "check_availability", description: "Check availability of a specific book by title or ID", inputSchema: { type: "object", properties: { book_id: { type: "string" }, title: { type: "string" } } } },
      { name: "get_due_dates", description: "Get upcoming due dates for borrowed books", inputSchema: { type: "object", properties: { student_id: { type: "string" } } } },
    ],
    invoke: (tool, input) => {
      ...
      if (tool === "check_availability") {
        ...
        return { found: true, book, status: book.available > 0 ? "Available" : "All copies checked out", availableCopies: book.available, totalCopies: book.total };
      }
  ...
  cafeteria: {
    ...
    tools: [
      { name: "get_menu", description: "Get the cafeteria menu", inputSchema: { type: "object", properties: { day: { type: "string" }, meal: { type: "string", enum: ["breakfast", "lunch", "dinner", "all"] } } } },
      { name: "get_specials", description: "Get today's special offers", inputSchema: { type: "object", properties: { day: { type: "string" } } } },
      { name: "get_hours", description: "Get cafeteria hours", inputSchema: { type: "object", properties: {} } },
    ],
    invoke: (tool, input) => {
  ...
  events: {
    ...
    description: "Browse and search campus events",
    tools: [
      { name: "list_events", description: "List all campus events", inputSchema: { type: "object", properties: { category: { type: "string" }, date: { type: "string" }, upcoming_only: { type: "boolean" } } } },
      ...
      { name: "get_event_details", description: "Get event details", inputSchema: { type: "object", properties: { event_id: { type: "string" } }, required: ["event_id"] } },
    ],
    invoke: (tool, input) => {
      if (tool === "list_events") {
        let results = [...events];
  ...
  academics: {
    ...
    tools: [
      { name: "get_deadlines", description: "Get upcoming deadlines", inputSchema: { type: "object", properties: { course_code: { type: "string" }, type: { type: "string" }, days_ahead: { type: "number" } } } },
      { name: "search_courses", description: "Search courses", inputSchema: { type: "object", properties: { query: { type: "string" }, semester: { type: "number" } }, required: ["query"] } },
      { name: "get_handbook", description: "Search handbook policies", inputSchema: { type: "object", properties: { query: { type: "string" } }, required: ["query"] } },
    ],
    invoke: (tool, input) => {
...
// Direct invoke without HTTP — for internal use
export function invokeMCPDirect(serverId: string, tool: string, input: Record<string, unknown>): { success: boolean; tool: string; result: unknown; error?: string } {
  const srv = mcpServers[serverId];
  if (!srv) return { success: false, tool, result: null, error: `Unknown server: ${serverId}` };
  try {
